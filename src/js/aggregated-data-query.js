...
function createAggregationTemplate(suffix) {
  return {
    ...
    median_apc_amount: { 
      percentiles: { field: "supplements.apc_cost", percents: [50] } 
    },
    with_data: { 
      filter: { term: { 
    ...
    unknown_shared_data_status: {
      filter: { bool: { must_not: { exists: { field: "supplements.dev.data.has_shared_data" } } } },
    },
    with_shared_data_in_repository: {
      filter: {
        bool: {
          must: [
            { term: { "supplements.dev.data.has_shared_data": true } },
            { terms: { "supplements.dev.data.evidence": ["da", "fulltext"] } },
          ],
          should: [
    ...
    with_open_data: {
      filter: {
        bool: {
          must: [
            { term: { "supplements.dev.data.has_open_data": true } },
            { terms: { "supplements.dev.data.evidence": ["da", "fulltext"] } },
          ],
        },
      },
    ...
    with_code: {
      filter: {
        bool: {
          must: [
            { term: { "supplements.dev.code.has_made_code": true } },
            { term: { "supplements.is_original_research": true } },
          ],
        },
      },
    },
    without_code: {
      filter: {
        bool: {
          should: [
            { term: { "supplements.dev.code.has_made_code": false } },
            { term: { "supplements.is_original_research": false } },
          ],
          minimum_should_match: 1,
        },
      },
    ...
    unknown_code_status: {
      filter: { bool: { must_not: { exists: { field: "supplements.dev.code.has_made_code" } } } },
    },
    with_shared_code: {
      filter: {
        bool: {
          must: [
            { term: { "supplements.dev.code.has_shared_code": true } },
            { terms: { "supplements.dev.code.evidence": ["da", "fulltext"] } },
          ],
        },
      },
    },
    without_shared_code: {
      filter: {
        bool: {
          should: [
            { term: { "supplements.dev.code.with_shared_code": false } },
            {
              bool: {
                must: [{ term: { "supplements.dev.code.evidence": "supplementary" } }],
                must_not: { terms: { "supplements.dev.code.evidence": ["da", "fulltext"] } },
              },
            },
          ],
          minimum_should_match: 1,
    ...
    unknown_shared_code_status: {
      filter: { bool: { must_not: { exists: { field: "supplements.dev.code.has_shared_code" } } } },
    },
    ...
    with_open_code: {
      filter: {
        bool: {
          must: [
            { term: { "supplements.dev.code.has_open_code": true } },
            { terms: { "supplements.dev.code.evidence": ["da", "fulltext"] } },
          ],
        },
      },
    ...
    with_orcids: { filter: { exists: { field: "authorships.author.orcid" } } },
    with_rors: { filter: { exists: { field: "authorships.institutions.ror" } } },
    with_fundref_dois: { filter: { exists: { field: "funder.DOI" } } },
    with_grant_dois: { filter: { prefix: { "funder.award": "10." } } },
  };
}
